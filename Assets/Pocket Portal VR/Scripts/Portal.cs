...
/* Gear VR Special */
	...
	private void RenderOpenVR(Camera camera) {
        ...
        Vector3 leftPosition = trackingSpace.TransformPoint(InputTracking.GetLocalPosition(XRNode.LeftEye));
        Vector3 rightPosition = trackingSpace.TransformPoint(InputTracking.GetLocalPosition(XRNode.RightEye));

        Quaternion leftRot = trackingSpace.transform.rotation * InputTracking.GetLocalRotation(XRNode.LeftEye);
        Quaternion rightRot = trackingSpace.transform.rotation * InputTracking.GetLocalRotation(XRNode.RightEye);
